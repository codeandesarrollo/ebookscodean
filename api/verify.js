...
// Inicializa Stripe con tu clave secreta (configurada en las variables de entorno en Vercel)
...
// Usamos la misma clave secreta para firmar el token JWT (configúrala en Vercel como JWT_SECRET)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (session.payment_status === 'paid') {
      // Genera un token único que expira en 1 hora (ajusta según tus necesidades)
      ...
      const downloadUrl = `https://TU-PROYECTO.vercel.app/api/descarga?token=${downloadToken}`;
